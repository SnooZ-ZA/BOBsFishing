...
-- T H R E A D
RegisterNetEvent('rod:onUse')
AddEventHandler('rod:onUse', function(source)
		...
		while CFish do
			...
			if IsControlJustPressed(1, Caught_KEY) then
				if BarAnimation >= SuccessLimit then
					...
					TriggerServerEvent('fishing:cought')
					StopAnimTask(GetPed(), 'amb@world_human_stand_fishing@idle_a','idle_c',2.0)
					Citizen.Wait(200)
					DeleteEntity(FishRod)
				else
...
function Chat(text)
	TriggerEvent("chatMessage", 'FISHING', { 255,255,0}, text)
end
